...
static long pn547_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case PN547_GET_IRQ:
		irq_status = gpio_get_value(pn547_dev->irq_gpio);
		if (copy_to_user((int __user *)arg, &irq_status ,sizeof(irq_status))) {
...
#ifdef CONFIG_COMPAT
static long pn547_dev_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct pn547_dev *pn547_dev = filp->private_data;
	int irq_status;
...
#ifdef CONFIG_COMPAT
    .compat_ioctl	= pn547_dev_ioctl_compat,
#endif
};
		...
		const struct i2c_device_id *id)
{
    ...
    if( client ->dev.of_node ){
        platform_data->irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "nxp_nfc,irq-gpio", 0, NULL);
        platform_data->ven_gpio = of_get_named_gpio_flags(client->dev.of_node, "nxp_nfc,ven-gpio", 0, NULL);
        platform_data->firm_gpio = of_get_named_gpio_flags(client->dev.of_node, "nxp_nfc,firm-gpio", 0, NULL);
	}
	 ...
	 */
	pr_info("%s : requesting IRQ %d, addr %x\n", __func__, client->irq, client->addr);
	pn547_dev->irq_enabled = true;
...
MODULE_DEVICE_TABLE(of, nxpnfc_i2c_dt_match);
